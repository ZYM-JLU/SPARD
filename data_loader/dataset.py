...
class Dataset:
    ...
    def sample(self):
        ...
        return traj[None, ...], subject, action, fr_start

    def chose_sample(self,subject = None,action = None,fr_start = None):
        if not subject:
            subject = np.random.choice(self.subjects)
        dict_s = self.data[subject]
        if not action:
    ...
    def sampling_generator(self, num_samples=1000, batch_size=8, aug=True):
        for i in range(num_samples // batch_size):
            ...
            for i in range(batch_size):
                sample_i, subject_i, action_i, fr_start_i = self.sample()
